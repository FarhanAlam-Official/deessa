...
async function confirmConferenceRegistrationFromWebhook(
  ...
  if (reg.payment_status === "paid" || reg.status === "confirmed" || reg.status === "cancelled") {
    return
  }

  // Fail-closed: only proceed when Stripe confirms the session is paid
  if (session.payment_status !== "paid") {
    console.warn("Stripe webhook (conference): session not paid", { registrationId, sessionId: session.id })
    return
  }
  ...
  if (reg.payment_amount !== null && session.mode === "payment") {
    ...
    if (actualMinor === null) {
      console.error("Stripe webhook (conference): no amount_total in session — flagging for review", {
    ...
    if (expectedMinor !== actualMinor) {
      console.error("Stripe webhook (conference): amount mismatch — flagging for review", {
        registrationId, expectedMinor, actualMinor, regCurrency, sessionCurrency,
      })
      await supabase
        .from("conference_registrations")
        .update({ payment_status: "review", stripe_session_id: session.id })
    ...
    if (regCurrency !== sessionCurrency) {
      console.warn("Stripe webhook (conference): currency mismatch — syncing DB currency to Stripe", {
  ...
  }).catch((err) => console.error("Non-fatal: webhook confirmation email failed:", err))

  console.log("Stripe webhook (conference): confirmed registration", registrationId)
}
  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const conferenceRegistrationId = session.metadata?.conference_registration_id
        if (conferenceRegistrationId) {
          await confirmConferenceRegistrationFromWebhook(supabase, conferenceRegistrationId, session, event.id)
          break
        }
      ...
      case "checkout.session.expired": {
        ...
        if (conferenceRegistrationId) {
          ...
          console.log("Stripe webhook (conference): payment session expired (no action)", conferenceRegistrationId)
          break
        }
